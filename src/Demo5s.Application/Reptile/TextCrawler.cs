...
{
    public class TextCrawler : ApplicationService
    {
        //写死的Url
        ...
        public List<ReptileModel> GetUrl()
        {
            ...
            //获取一整页数据 从前台传过来的Url
            var html = Crawler.DownLoadUrl(_Url);//封装的静态类
            List<ReptileModel> list = new List<ReptileModel>();
            ...
            {
                var goods = Crawler.GoodsList(Node);//调用 当个节点的值
